...
save_dir = f"../checkpoints/checkpoints_260427_EMA_Xattn_MAB{k}_semi_z_lam{lam}_sab{n_sab}_grid_proj_LeJ"

mab_transformer = FovealSetTransformer(input_dim=embed_dim, 
                 n_heads=n_heads, n_sab=n_sab, k=k, proj=True, proj_dim=proj_dim, predict=False)
                 #n_heads=12, n_sab=4, predict=False)
mab_transformer.to(device)
mab_transformer.train()

linear_head = nn.Sequential(
                #nn.LayerNorm(k*proj_dim),
                nn.Linear(k*proj_dim, 1000),
            )
    ...
    for batch_idx, (features, sxs, sys_, labels) in enumerate(pbar):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            for i in range(n_student_draws):
                ...
                loss_sigreg += sigreg(output_s[i,...].view(batch_size, k * proj_dim).float(), global_step)
            ...
            output_t_head = linear_head(centers.view(batch_size, k*proj_dim).detach()) #linear_head(output_t[0].detach()) + linear_head(output_t[1].detach())
