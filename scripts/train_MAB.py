...
save_dir = "../checkpoints/checkpoints_260427_EMA_Xattn_MAB"
...
mab_predictor = FovealSetTransformer(input_dim=embed_dim, 
                 n_heads=8, n_sab=2, n_classes=1000)
mab_predictor.to(device)
mab_predictor.train()
    ...
    for batch_idx, (views, sxs, sys_, zooms, labels) in enumerate(pbar):
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            torch.save({
                    "epoch": epoch,
                    "history": history,
                    "dual_predictor": mab_predictor.state_dict(),
