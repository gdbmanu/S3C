...
for epoch in range(train_epochs):  # 20-30 époques suffisent
    ...
    for batch_idx, (mosaics, sxs, sys_, zooms, labels) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast():
            shift = torch.stack([sxs[:,1] - sxs[:,0], sys_[:,1] - sys_[:,0]], dim=1)
            with torch.no_grad():
                features_1 = model(mosaics[:, 0])  # Extraction des features
                features_2 = model(mosaics[:, 1])  # Extraction des features
            pred_shift, output = dual_predictor(features_1[:, 0], features_2[:, 0])
            loss_shift = mse(pred_shift, shift)
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            with torch.no_grad():
                for _ in range(5):
                    ...
                    shift = torch.stack([sxs[:,1] - sxs[:,0], sys_[:,1] - sys_[:,0]], dim=1)
                    features_1 = model(mosaics[:, 0]) 
                    features_2 = model(mosaics[:, 1])
            ...
            torch.save({
                    "epoch": epoch,
                    "history": history,
                    "dual_predictor": dual_predictor.state_dict(),
